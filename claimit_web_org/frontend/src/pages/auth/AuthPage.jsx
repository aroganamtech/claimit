...
export default function AuthPage({ portal }) {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        try {
          ...
          setError('Could not look up address from your location')
  ...
  const handleSendOtp = async () => {
    ...
    setLoading(true)
    try {
      const data = await api.auth.sendOtp({ phone, email, role: config.role })
  ...
  const handleCompleteReg = async () => {
    ...
    try {
      const data = await api.auth.register({
        phone, email, name, address, pincode, lat, lng, role: config.role
      })
      login(data)
