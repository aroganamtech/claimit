...
async def register_shop(
    ...
    # Razorpay payment proof (empty for a free ₹0 "other" plan)
    ...
    geo_point = {"type": "Point", "coordinates": [lng, lat]} if (lat is not None and lng is not None) else None
    ...
    # Record the transaction (full shop + user + payment details) in its own
    # collection so the admin can see every registration & payment.
    try:
        await transactions_collection.insert_one({
            ...
            "shop": {
                ...
                "shop_type": shop_type, "discount_percentage": discount_percentage,
                "location": location, "address": shop_address, "pincode": pincode,
                "phone": phone, "timing": timing,
            ...
            "user": {
                ...
                "email": user_email,
                "phone": current_user.get("phone", "") or phone,
            },
            "plan":                (plan or "").strip().lower(),
            "amount":              amt,
            "currency":            "INR",
            ...
            "payment_status":      "paid" if amt > 0 else "free",
            "created_at":          datetime.utcnow(),
        })
    except Exception as e:
        print(f"⚠️  transaction record failed: {e}")
...
async def create_shop_pay_order(body: PayOrderBody,
                                current_user=Depends(get_current_user)):
...
async def claim_shop(body: ClaimShopBody, current_user=Depends(get_current_user)):
    ...
    # Record the transaction (full shop + user + payment details), same as a
    # fresh /shop/register, so admin sees every claim & payment too.
    try:
        await transactions_collection.insert_one({
            "type":        "shop_claim",
            ...
            "shop": {
                "shop_name": updates["shop_name"], "category": shop.get("category", ""),
                "shop_type": st, "discount_percentage": disc,
                "location": shop.get("location", ""), "address": updates["shop_address"],
                "pincode": shop.get("pincode", ""), "phone": shop.get("phone", ""),
                "timing": shop.get("timing", ""),
                "country": shop.get("country", ""), "state": shop.get("state", ""),
                "district": shop.get("district", ""), "city": shop.get("city", ""),
            },
            "user_id":     str(current_user["_id"]),
            ...
            "user": {
                ...
                "phone": current_user.get("phone", "") or shop.get("phone", ""),
            },
            "plan":                updates["plan"],
            "amount":              amt,
            "currency":            "INR",
        ...
        print(f"⚠️  transaction record failed (claim): {e}")
