...
from database import ads_collection, users_collection, transactions_collection
from models.schemas import AdType
...
async def create_ad(
    ...
    # ── Deal fields (nearby_deals / brand_deals) ──────────────────────────────
    ...
    type: Optional[str] = Form(None),          # category e.g. Restaurant
    ...
    if creative and creative.filename:
        safe_name = f"{user_id}_{int(datetime.utcnow().timestamp())}_{creative.filename}"
        file_path = os.path.join(upload_dir, safe_name)
        content = await creative.read()
        with open(file_path, "wb") as f:
            f.write(content)
        creative_url = f"/uploads/{safe_name}"

    thumbnail_url = None
    if thumbnail and thumbnail.filename:
        safe_name = f"{user_id}_{int(datetime.utcnow().timestamp())}_thumb_{thumbnail.filename}"
        file_path = os.path.join(upload_dir, safe_name)
        content = await thumbnail.read()
        with open(file_path, "wb") as f:
            f.write(content)
        thumbnail_url = f"/uploads/{safe_name}"
    ...
    if ad_type == "home_banner":
        ad_doc.update({
            ...
            "description": sub or description or "",
        })

    elif ad_type == "promo_reelz":
        ad_doc.update({
            "shop_name": shop_name or title or "",
            "shop_location": shop_location or location or "",
            "shop_category": shop_category or type or "",
            "caption": caption or description or "",
            "offer": offer or "",
            "tag": tag or "",
            ...
            "liked_by_me": False,
            # legacy compat
            "title": shop_name or title or "",
            "description": caption or description or "",
        })

    elif ad_type in ("brand_deals", "nearby_deals"):
        ad_doc.update({
            "name": name or shop_name or title or "",
            "location": location or shop_location or "",
            "offer": offer or "",
            "description": description or caption or "",
            "address": address or "",
            "phone": phone or "",
            "timing": timing or "",
            "type": type or shop_category or "",
    ...
    await transactions_collection.insert_one({
        ...
        "type": "ad_purchase",
        "ad_id": ad_id,
        "title": ad_doc.get("title") or ad_doc.get("name") or ad_doc.get("shop_name") or ad_doc.get("headline") or "",
        "amount": amount,
