...
export default function SalesSidebar() {
  ...
  useEffect(() => {
    api.get('/sales/profile').then(r => setProfile(r.data)).catch(() => {})
  }, [])
  ...
  return (
    <aside className="sidebar">
      <div style={{ padding: '20px', borderBottom: '1px solid #eee', display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div>
          <div style={{ fontWeight: 600, fontSize: 14 }}>{user?.name || 'Dhanush'}</div>
          <div style={{ fontSize: 12, color: '#888' }}>User Id : {profile?.user_id || '528882'}</div>
        </div>
      </div>
