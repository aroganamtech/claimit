...
import '../../shops/screens/shop_list_screen.dart'; // ShopItem + getShopDatabase()
import '../../../shared/widgets/shop_filter_sheet.dart';

// ─────────────────────────────────────────────────────────────────────────────
// SearchScreen
// 3 states: empty (recent + popular) | typing (suggestions) | results
// ─────────────────────────────────────────────────────────────────────────────
...
class _SearchScreenState extends State<SearchScreen> {
  ...
  List<ShopItem> get _results {
    ...
    List<ShopItem> shops = getShopDatabase()
        .where((s) =>
  ...
  Future<void> _initSpeech() async {
    final available = await _speech.initialize(
      onStatus: (status) {
        ...
        // Auto-stop when the recognizer finishes
  ...
  void _showSpeechUnavailableSnack() {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: const Text('Speech recognition is not available on this device.'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      // Listening banner shown below the appbar when mic is active
      body: Stack(
        children: [
          showResults
  ...
  // ── AppBar — same white style as home/dashboard ───────────────────────────
              ...
              Padding(
                ...
                child: Row(
                  children: [
                    ...
                    const Spacer(),
                    // Notification icon — use go() because /notifications is
                    // a ShellRoute child; push() would cause a duplicate-key crash.
                    GestureDetector(
                      ...
                      child: Container(
                        ...
                        child: Row(
                          children: [
                            ...
                            // Mic button — animated when listening
                            GestureDetector(
                              onTap: _toggleListening,
                              child: Container(
  ...
  Widget _buildSuggestions() {
    // Show recent + popular that match query
    final q = _query.toLowerCase();
    final suggestions = [
      ..._recentSearches.where((s) => s.toLowerCase().contains(q)),
      ..._popularSearches.where((s) =>
          s.toLowerCase().contains(q) && !_recentSearches.contains(s)),
    ];
    ...
    return ListView.builder(
      ...
      itemBuilder: (_, i) {
        ...
        return _RecentRow(
          ...
          onRemove: _recentSearches.contains(s)
              ? () => _removeRecent(s)
              : null,
        );
      },
        ...
        Container(
          ...
          child: Row(
            children: [
              ...
              const Spacer(),
              // Filter button — wired to ShopFilterSheet
              GestureDetector(
                onTap: _openFilter,
                child: Container(
                  padding:
...
class _RecentRow extends StatelessWidget {
  ...
  const _RecentRow(
      {required this.text, required this.onTap, this.onRemove});
