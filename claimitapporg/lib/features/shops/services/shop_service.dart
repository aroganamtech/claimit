...
class ShopService {
  ...
  /// unchanged.
  /// [pincode], [city], [area], [district] — the user's address, used by the
  /// server to also return shops that have NO stored coordinates (registered
  /// without "use my current location", or bulk-uploaded with no lat/lng
  /// columns). Those can't take part in the distance query, so they are
  /// matched on address and appended after the distance-sorted results.
  /// All optional: when omitted the server falls back to the signed-in
  /// user's saved profile address.
    ...
    try {
      final resp = await _api.get(
        ...
        queryParams: {
          ...
          if (excludeId != null) 'exclude_id': excludeId,
          if (has(pincode)) 'pincode': pincode!.trim(),
          if (has(city)) 'city': city!.trim(),
          if (has(area)) 'area': area!.trim(),
          if (has(district)) 'district': district!.trim(),
        },
      );
      if (resp.statusCode == 200 && resp.data is Map) {
        final list = resp.data['shops'] as List? ?? [];
