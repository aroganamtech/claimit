...
async def list_my_classifieds(
    ...
    """The caller's own posts. `listing_type` keeps Local Finds and Local
    Classifieds apart — they share this collection, so without the filter a
    user's businesses show up inside Classifieds and vice versa. Omitting it
    still returns everything (kept for backwards compatibility)."""
    db = get_db()
