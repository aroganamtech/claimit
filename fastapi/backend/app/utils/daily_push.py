...
from .whatsapp import broadcast_notification
...
async def _broadcast(title: str, body: str) -> None:
    ...
    then the same message over WhatsApp."""
    db = get_db()
    ...
    # Same message over WhatsApp, to every user who has a phone number.
    #
    # Dormant until TWILIO_NOTIFY_TEMPLATE_SID is set. Note this one costs
    # money per message and goes to the whole user base, so watch the spend
    # and the WhatsApp quality rating in the Twilio console after enabling it.
    try:
        await broadcast_notification(db, title, body)
    except Exception as e:
        print(f"⚠️  WhatsApp daily broadcast skipped: {e}")
