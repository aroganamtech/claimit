...
async def _backfill_shop_geo(database) -> None:
    """
    One-time (idempotent, re-run-safe) backfill: every shop that has lat/lng
    but no GeoJSON `geo` point yet gets one. Needed so /shops/nearby can use
    a real $geoNear query against a 2dsphere index instead of pulling every
    shop into memory and computing distance in Python. Only touches docs
    that are missing `geo` — a no-op on subsequent restarts once caught up.
    """
    cursor = database.shops.find(
        {
            "lat": {"$ne": None},
            "lng": {"$ne": None},
            "geo": {"$exists": False},
    ...
    async for doc in cursor:
        ...
        updated += 1
    if updated:
        print(f"✅ Backfilled geo point on {updated} shop(s)")
...
async def connect_db():
    ...
    # Geospatial index for /shops/nearby ($geoNear) — backfill existing rows
    # first so the index has something to serve immediately after deploy.
    await _backfill_shop_geo(db)
    await db.shops.create_index([("geo", "2dsphere")])
    # Deals
