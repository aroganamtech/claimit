...
NOTE (Vercel): the serverless filesystem is read-only/ephemeral and your
deployment does NOT include a `secret/` folder, so set
FIREBASE_SERVICE_ACCOUNT_JSON (paste the full service-account JSON as one env
var value) in the Vercel project settings rather than relying on
...
def _build_credentials():
    ...
    if settings.firebase_service_account_path:
        ...
        if not os.path.isabs(path):
            # Resolve relative to the backend root (fastapi/backend/vercel_backend/),
            # i.e. two levels up from this file (app/utils/fcm.py -> vercel_backend/).
        ...
        try:
            ...
            if cred is None:
                print("⚠️  FCM: no service account configured — push notifications disabled "
                      "(set FIREBASE_SERVICE_ACCOUNT_JSON in Vercel env vars)")
                return None
